...
export default (state={}, action) => {
  ...
  //organize trasms by tournament, classifiaction, amd agegroup
  takeAction['TEAM_SET_ALL'] = (payload) => {
    let[tournamentId, teams] = payload;
    let tempState = {...state};
    teams.forEach(team => {
      if (!tempState[tournamentId]) tempState[tournamentId] = {};
      if (!tempState[tournamentId][team.classification]) tempState[tournamentId][team.classification] = {};
      //convert birthyear to ageGroup
      let ageGroup = `U${thisYear - team.birthyear}`;
      if (!tempState[tournamentId][team.classification][ageGroup]) tempState[tournamentId][team.classification][ageGroup] = [];
